...
resource "azurerm_public_ip" "pnp_hub_bastion_vm_ip" {
  ...
  resource_group_name = var.shared_var.hub_resource_group_name # 리소스 그룹 이름
  location            = var.shared_var.location # 공용 IP 위치
...
resource "azurerm_network_interface" "pnp_hub_bastion_vm_nic" {
  ...
  resource_group_name = var.shared_var.hub_resource_group_name # 네트워크 인터페이스가 속한 리소스 그룹 이름 
  location            = var.shared_var.location # 네트워크 인터페이스 위치
...
resource "azurerm_network_security_group" "pnp_hub_bastion_vm_nsg" {
  ...
  resource_group_name = var.shared_var.hub_resource_group_name   # NSG가 속한 리소스 그룹 이름
  location            = var.shared_var.location              # NSG 위치
}
...
resource "azurerm_windows_virtual_machine" "PNP-HUB-BASTION-VM" {
  ...
  resource_group_name = var.shared_var.hub_resource_group_name # 가상 머신이 속한 리소스 그룹 이름
  location            = var.shared_var.location # 가상 머신 위치
