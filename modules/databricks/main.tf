...
variable "pnp_hub_adb_auth_nsg_id" {}
variable "pnp_spoke_adb_backend_nsg_id" {}
...
resource "azurerm_databricks_workspace" "pnp_d_kr_hub_adb_auth" {
  ...
  custom_parameters {
    ...
    public_subnet_network_security_group_association_id = var.pnp_hub_adb_auth_nsg_id
    private_subnet_network_security_group_association_id = var.pnp_hub_adb_auth_nsg_id
  }
...
resource "azurerm_databricks_workspace" "pnp_d_kr_spoke_adb_backend" {
  ...
  custom_parameters {
    ...
    public_subnet_network_security_group_association_id = var.pnp_spoke_adb_backend_nsg_id
    private_subnet_network_security_group_association_id = var.pnp_spoke_adb_backend_nsg_id
  }
...
resource "azurerm_private_endpoint" "pnp_hub_adb_auth_browser_pep" {
  name                = "${var.conv.project_name}-hub-adb-browser-pep"                   # Private Endpoint 이름
...
# Azure Databricks Frontend Browser Private Endpoint 생성
resource "azurerm_private_endpoint" "pnp_hub_adb_auth_api_pep" {
  name                = "${var.conv.project_name}-hub-adb-api-pep"                   # Private Endpoint 이름
  resource_group_name = var.com_var.hub_resource_group_name   # Private Endpoint가 속한 리소스 그룹 이름
  location            = var.com_var.location  # Private Endpoint 위치
  subnet_id           = var.pnp_hub_adb_pep_subnet_id # Private Endpoint를 할당할 서브넷 ID
...
resource "azurerm_private_endpoint" "pnp_spoke_adb_backend_api_pep" {
  name                = "${var.conv.project_name}-spoke-adb-backend-api-pep"                   # Private Endpoint 이름
  resource_group_name = var.com_var.spoke_resource_group_name   # Private Endpoint가 속한 리소스 그룹 이름
  location            = var.com_var.location  # Private Endpoint 위치
  subnet_id           = var.pnp_spoke_adb_pep_subnet_id # Private Endpoint를 할당할 서브넷 ID

  # 연결된 서비스 구성
  private_service_connection {
    name                           = "api-adb-pe-connection"  # Private Endpoint 연결 이름
    private_connection_resource_id = azurerm_databricks_workspace.pnp_d_kr_spoke_adb_backend.id # Private Endpoint 연결 리소스 ID
    is_manual_connection           = false                          # 수동 연결 여부
    subresource_names = ["databricks_ui_api"] # Private Endpoint 하위 리소스 
  }
